...
async def login_handler(c: Client, m: Message):
    try:
        try:
            jv = await m.reply_text("Now send me password.\n\n If You don't know check the  in Our support group @thanimaisupport or else type this `Thanimaibots` \n\n(You can use /cancel command to cancel the process)")
        ...
        if textp == MY_PASS:
            ...
            jv_text = "Wrong password, conact @thanimaisupport "
...
async def private_receive_handler(c: Client, m: Message):
    ...
    if check_pass== None:
        await m.reply_text("Login first using /login cmd \n don\'t know the pass? request it from @sarbudeen786 ")
        return
    ...
    if Var.UPDATES_CHANNEL != "None":
        try:
            ...
            if user.status == "kicked":
                await c.send_message(
                    chat_id=m.chat.id,
                    text="__Sᴏʀʀʏ Sɪʀ, Yᴏᴜ ᴀʀᴇ Bᴀɴɴᴇᴅ ᴛᴏ ᴜsᴇ ᴍᴇ.__\n\n  **Cᴏɴᴛᴀᴄᴛ Dᴇᴠᴇʟᴏᴘᴇʀ @sarbudeen786 ʜᴇ Wɪʟʟ Hᴇʟᴘ Yᴏᴜ**",
                    parse_mode="markdown",
            ...
            await c.send_message(
                chat_id=m.chat.id,
                text="**Sᴏᴍᴇᴛʜɪɴɢ ᴡᴇɴᴛ Wʀᴏɴɢ. Cᴏɴᴛᴀᴄᴛ ᴍʏ ʙᴏss** @sarbudeen786",
                parse_mode="markdown",
                disable_web_page_preview=True)
            return
    try:
...
async def channel_receive_handler(bot, broadcast):
    ...
    if check_pass == None:
        await broadcast.reply_text("Login first using /login cmd \n don\'t know the pass? request it from @sarbudeeb786")
        return
    if check_pass != MY_PASS:
